...
<trades>
    <order>
        ...
        <price>176.20</price>
        <trade_date>2024-12-01</trade_date>
    </order>
    <order>
        <client_id>2</client_id>
        <stock_symbol>TSLA</stock_symbol>
        <type>SELL</type>
        <quantity>5</quantity>
        <price>248.75</price>
        <trade_date>2024-12-02</trade_date>
    </order>
</trades>`;
...
export default function TradeOpsApp() {
  ...
  // NEW: State to toggle sample XML visibility
  ...
  return (
    <div className="min-h-screen w-full bg-linear-to-br from-slate-900 via-slate-800 to-slate-900 font-sans text-slate-200 overflow-x-hidden">
      {/* Container - removed nested wrapper */}
      <div
        id="dashboard"
        className="w-full max-w-full  mx-auto px-3 sm:px-6 lg:px-8 py-4"
      >
        {/* Navbar */}
        {/* UPDATED: Reduced margins and padding for mobile to prevent "zoomed" feel */}
        <nav className="sticky top-0 z-50 backdrop-blur-md bg-slate-900/80 border-b border-slate-800 mb-6 rounded-xl sm:rounded-none -mx-3 sm:-mx-6 lg:-mx-8 px-3 sm:px-6 lg:px-8">
        ...
        <div className="bg-slate-800/40 backdrop-blur border border-slate-700/60 rounded-xl p-4 sm:p-6 mb-8 w-full">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 gap-2">
            <h2 className="text-lg font-semibold text-white flex items-center gap-2">
              <Upload className="w-5 h-5 text-emerald-400" />
              Process Trade Settlement
            </h2>
            {/* NEW: Button to toggle Sample XML */}
            <button
              onClick={() => setShowSample(!showSample)}
              className="text-xs flex items-center gap-1 text-slate-400 hover:text-emerald-400 transition-colors"
            >
              <FileText className="w-3 h-3" />
              {showSample ? "Hide Sample Format" : "View Sample XML Structure"}
            </button>
          </div>

          {/* NEW: Sample XML Display Area */}
          {showSample && (
            <div className="mb-4 bg-slate-900/80 rounded-lg border border-slate-700 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200 w-full">
              <div className="px-4 py-2 bg-slate-800/50 border-b border-slate-700 flex items-center justify-between">
                ...
                <button
                  onClick={() => {
                    navigator.clipboard.writeText(SAMPLE_XML);
                  }}
              ...
              <pre className="p-4 text-xs sm:text-sm text-slate-300 font-mono overflow-x-auto">
                {SAMPLE_XML}
              </pre>
            </div>
          )}
          ...
          <div className="flex flex-col lg:flex-row items-stretch gap-4 w-full">
            ...
            <button
              ...
              className="w-full lg:w-auto lg:min-w-[180px] px-6 py-3 bg-linear-to-r from-emerald-600 to-emerald-500
                ...
                flex items-center justify-center gap-2 transform active:scale-95 whitespace-nowrap"
            >
              {loading ? (
                <>Processing...</>
              ) : (
        ...
        <div className="w-full bg-slate-800/40 backdrop-blur border border-slate-700/60 rounded-xl overflow-hidden shadow-xl">
          ...
          <table className="w-full table-fixed whitespace-nowrap text-sm">
            <thead className="bg-slate-900/50">
              <tr className="text-left text-xs font-semibold text-slate-400 uppercase tracking-wider border-b border-slate-700/50">
                <th className="px-4 sm:px-6 py-3 w-12">ID</th>
                <th className="px-4 sm:px-6 py-3 w-24">Client</th>
                <th className="px-4 sm:px-6 py-3 w-20">Stock</th>
                <th className="px-4 sm:px-6 py-3 w-16">Type</th>
                <th className="px-4 sm:px-6 py-3 w-24 text-right">Value</th>
                <th className="px-4 sm:px-6 py-3 w-24">Date</th>
                <th className="px-4 sm:px-6 py-3 w-20 text-center">Status</th>
              </tr>
            </thead>
